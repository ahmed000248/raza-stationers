...
export default function StockManagementPage() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="font-heading text-2xl font-bold">Stock Management</h1><p className="mt-1 text-xs text-[var(--text-muted)]">Unknown opening stock is distinct from a confirmed zero quantity. Every change records its before, delta, after, reason, time and actor.</p></div>
          ...
          <table className="w-full min-w-[820px] text-left text-xs">
            ...
            <tbody className="divide-y divide-[var(--border-subtle)]">{items.map((item) => {
              ...
              return <tr key={item.id}>
                ...
                <td className="max-w-80 px-4 py-3 text-[var(--text-muted)]">{item.lastMovement ? <><p>{previous ?? "-"} {delta == null ? "" : `${delta >= 0 ? "+" : ""}${delta}`} to {next ?? "-"}</p><p className="mt-1">{item.lastMovement.reason} / {item.lastMovement.createdBy.name} / {new Date(item.lastMovement.occurredAt).toLocaleString("en-PK")}</p></> : "No ledger entry"}</td>
                <td className="px-4 py-3 text-right">{canEdit && <Button type="button" variant="outline" className="h-9" onClick={() => open(item)}>{item.stockState === "not_initialized" ? "Enter opening stock" : "Adjust"}</Button>}</td>
              </tr>
            })}</tbody>
          </table>
        </div>
