...
async function main() {
    ...
    if (!sourcePath) {
        sourcePath = node_path_1.default.join(process.cwd(), ".codex-phase7-tmp", "catalogue-products.csv");
    }
    if (!dryRun && !commit) {
    ...
    if (commit) {
        ...
        const { result: planResult } = await importer_js_1.CatalogueImporter.generatePlan(sourcePath);
        result = planResult;
    }
