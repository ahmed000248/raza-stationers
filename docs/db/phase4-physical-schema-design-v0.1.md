...
**Phase 5B reconciliation (2026-07-26):** Ahmed approved an optional base-unit low-stock threshold, stable yearly Order numbers, store-credit-only ledger semantics, explicit CreditNote sources and the public-schema Supabase defence-in-depth plan. The initial migration implements these additions without changing the deferred feature boundary below.
...
- Optional `lowStockThresholdBase`, measured in Product base units. `null` disables alerts, zero means out-of-stock-only and negative values are rejected. No real threshold values are currently approved.
...
Customer/store credit balance is calculated as the sum of signed `CreditLedgerEntry.amount`; it is never directly edited. Positive entries add usable customer-owned credit and negative entries consume or pay it out. Unpaid invoice debt is separately calculated from Invoice, non-reversed PaymentAllocation and CreditNote records; invoice charges never become store-credit entries. User/business links have start/end actors and timestamps so historical Orders keep their acting User after a link ends.
...
Orders use stable `RS-ORD-YYYY-000001` visible numbers allocated transactionally by document type and year; the number is independent from the Order CUID. Version 0.1 uses one Invoice per Order. Invoice visible numbers use `RS-INV-YYYY-000001`, generated by the same `DocumentSequence` approach. Credit notes use `RS-CN-YYYY-000001` and preserve original Invoice totals.

Every CreditNote declares exactly one source type: cancellation, return or manual adjustment. Cancellation and return sources require the corresponding relation and exclude the other; manual adjustment carries neither relation and requires future NestJS Owner-role validation.
